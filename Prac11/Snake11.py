...
FOOD_TYPES = [
    {"value": 10, "color": RED,    "lifetime": 80,  "label": "1x", "weight": 60},
    {"value": 30, "color": GOLD,   "lifetime": 50,  "label": "3x", "weight": 30},
    {"value": 50, "color": SILVER, "lifetime": 30,  "label": "5x", "weight": 10},
]
...
def pick_food_type():
    population = [ft for ft in FOOD_TYPES for _ in range(ft["weight"])]
    return dict(random.choice(population))
...
def new_food(snake_body):
    ft = pick_food_type()
    ft["pos"]     = random_food_pos(snake_body)
    ft["timer"]   = ft["lifetime"]
    ft["max_time"]= ft["lifetime"]
    return ft
...
def draw_food(surface, food):
    if not food:
        return
    fx = food["pos"][0] * CELL + CELL // 2
    fy = food["pos"][1] * CELL + HUD_H + CELL // 2

    ratio = food["timer"] / food["max_time"]
    pulse_r = int(CELL // 2 - 2 + 3 * ratio)

    pygame.draw.circle(surface, food["color"], (fx, fy), pulse_r)
    pygame.draw.circle(surface, WHITE, (fx - 3, fy - 3), 3)

    bar_w = int(CELL * ratio)
    pygame.draw.rect(surface, (80, 80, 80),
                     (food["pos"][0] * CELL, food["pos"][1] * CELL + HUD_H + CELL - 4,
                      CELL, 4))
    ...
    lbl = pygame.font.SysFont("Arial", 10, bold=True).render(
        food["label"], True, BLACK)
    surface.blit(lbl, (fx - lbl.get_width() // 2, fy - lbl.get_height() // 2))
...
def draw_hud(surface, score, level, speed):
    ...
    speed_surf = font_small.render(f"Speed: {speed}", True, GRAY)
