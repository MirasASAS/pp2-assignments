...
class Coin:
    def __init__(self, speed):
        population = [ct for ct in COIN_TYPES for _ in range(ct["weight"])]
        self.ctype  = random.choice(population)
        self.radius = self.ctype["radius"]
        self.value  = self.ctype["value"]
        self.color  = self.ctype["color"]
        self.shine  = self.ctype["shine"]
        lane = random.randint(0, 2)
    ...
    def draw(self, surface):
        ...
        label = pygame.font.SysFont("Arial", 10, bold=True).render(
...
def draw_hud(surface, score, coin_count, coin_score, enemy_speed):
    score_surf = font_small.render(f"Score: {score}", True, WHITE)
    surface.blit(score_surf, (10, 10))

    speed_surf = font_small.render(f"Enemy speed: {enemy_speed}", True, WHITE)
    surface.blit(speed_surf, (10, 36))

    coins_surf = font_medium.render(f"Coins: {coin_count}  +{coin_score}pts", True, YELLOW)
    surface.blit(coins_surf, (WIDTH - coins_surf.get_width() - 10, 10))

    next_thresh = ((coin_count // COIN_SPEED_THRESHOLD) + 1) * COIN_SPEED_THRESHOLD
    bar_surf = font_small.render(
        f"Next boost: {next_thresh - coin_count} coins", True, SILVER)
    surface.blit(bar_surf, (WIDTH - bar_surf.get_width() - 10, 46))
...
def main():
    ...
    show_screen(screen, "RACER", "SPACE to start  |  Arrow keys to move")
    waiting = True
    while waiting:
    ...
    while running:
        ...
        if not game_over:
            ...
            for coin in coins[:]:
                ...
                if coin.is_off_screen():
                    ...
                    score      += coin.value * 10
                    coins.remove(coin)
                    if coin_count % COIN_SPEED_THRESHOLD == 0:
                        enemy_speed += 1
                        speed_level += 1
        ...
        player.draw(screen)
        draw_hud(screen, score, coin_count, coin_score, enemy_speed)
